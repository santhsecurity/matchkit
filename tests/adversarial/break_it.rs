...
fn test_contains_edge_case_longer_end() {
    let m1 = Match::new(0, 5, 10);
    let m2 = Match::new(0, 5, 15);
...
fn test_maximum_u32_bounds() {
    let m = Match::new(u32::MAX, u32::MAX - 10, u32::MAX);
    assert_eq!(
        m.len(),
        10,
...
fn test_overlap_max_bounds() {
    let m1 = Match::new(0, u32::MAX - 5, u32::MAX);
    let m2 = Match::new(0, u32::MAX - 2, u32::MAX);
    assert!(
        m1.overlaps(&m2),
...
fn test_large_input_bounds() {
    ...
    let m = Match::new(1, 0, large_size as u32);
    assert_eq!(
        m.len(),
        large_size as u32,
...
fn test_concurrent_access_insert() {
    ...
    for i in 0..8 {
        ...
        handles.push(thread::spawn(move || {
            set.lock().unwrap().insert(Match::new(i, 0, 10));
        }));
    }
    for h in handles {
        h.join().unwrap();
...
fn test_concurrent_access_merge() {
    ...
    for i in 0..8 {
        ...
        handles.push(thread::spawn(move || {
            ...
            locked.insert(Match::new(i, i * 5, (i * 5) + 15));
...
fn test_duplicate_entries_try_insert() {
    let mut s = MatchSet::new();
    s.try_insert(Match::new(1, 10, 20)).unwrap();
    s.try_insert(Match::new(1, 10, 20)).unwrap();
    assert_eq!(
        s.len(),
        1,
...
fn test_off_by_one_first_byte() {
    let mut s = MatchSet::new();
    s.insert(Match::new(1, 0, 1));
    s.insert(Match::new(1, 1, 2));
    s.merge_overlapping();
    // Adjacent same-pattern matches are now coalesced to avoid fragmented findings.
    assert_eq!(s.len(), 1, "Adjacent same-pattern matches should merge");
    assert_eq!(s.as_slice()[0], Match::new(1, 0, 2));
}
...
fn test_merge_bound_inclusion() {
    let mut s = MatchSet::new();
    s.insert(Match::new(1, 0, 10));
    s.insert(Match::new(1, 9, 20));
    s.merge_overlapping();
...
fn test_resource_exhaustion_merge_100k() {
    let mut s = MatchSet::new();
    let matches: Vec<_> = (0..100_000).map(|i| Match::new(1, i, i + 2)).collect();
    s.extend(matches);
    s.merge_overlapping();
    assert_eq!(
        s.len(),
...
fn test_try_insert_exhaustion() {
    let mut s = MatchSet::new();
    let res = s.try_insert(Match::new(0, 0, 1));
...
// 23. Sort order edge cases
#[test]
fn test_match_ordering_edge_cases() {
    let mut s = MatchSet::new();
    // Insert reverse
    s.insert(Match::new(3, 10, 20));
    s.insert(Match::new(2, 10, 20));
    s.insert(Match::new(1, 10, 20));
...
// 24. Filter by pattern
#[test]
fn test_filter_by_pattern() {
    let mut s = MatchSet::new();
    s.insert(Match::new(1, 0, 10));
    s.insert(Match::new(2, 0, 10));
...
// 25. Pattern counts exactness
#[test]
fn test_pattern_counts_accumulation() {
    let mut s = MatchSet::new();
    s.insert(Match::new(5, 0, 10));
    s.insert(Match::new(5, 20, 30));
...
// 26. Pattern IDs exactness
#[test]
fn test_pattern_ids_deduplication() {
    let mut s = MatchSet::new();
    s.insert(Match::new(9, 0, 10));
    s.insert(Match::new(9, 20, 30));
    s.insert(Match::new(4, 10, 20));
...
// 27. Merge disjoint segments stability
#[test]
fn test_merge_disjoint_segments() {
    let mut s = MatchSet::new();
    s.insert(Match::new(1, 0, 10));
    s.insert(Match::new(1, 20, 30));
    s.merge_overlapping();
    assert_eq!(s.len(), 2, "Disjoint segments must not be merged");
}

// 28. Transitive contains boundary
#[test]
fn test_transitive_contains_boundary() {
    let m1 = Match::new(0, 0, 100);
    let m2 = Match::new(0, 50, 100);
    let m3 = Match::new(0, 50, 50);
...
// 29. Empty subset overlaps
#[test]
fn test_empty_subset_overlaps() {
    let s1 = MatchSet::new();
    let mut s2 = MatchSet::new();
    s2.insert(Match::new(1, 0, 10));
    assert!(s1.is_empty(), "New set is empty");
    assert!(!s2.is_empty(), "Set with insert is not empty");
}

// 30. Merge zero length matches
#[test]
fn test_merge_zero_length() {
    let mut s = MatchSet::new();
    s.insert(Match::new(1, 5, 5));
    s.insert(Match::new(1, 5, 10));
    s.merge_overlapping();
    assert_eq!(
        s.len(),
        1,
        "Adjacent same-pattern zero-length match merges with covering match"
    );
    assert_eq!(s.as_slice()[0], Match::new(1, 5, 10));
}

// 31. Multi-pattern merge precedence
#[test]
fn test_multi_pattern_merge_precedence() {
    let mut s = MatchSet::new();
    s.insert(Match::new(2, 0, 15));
    s.insert(Match::new(1, 5, 20));
    s.merge_overlapping();
    assert_eq!(
        s.as_slice()[0].pattern_id,
        2,
...
// 32. Try extend with empty iterator
#[test]
...
// 33. Match size must be exactly 12 bytes for GPU
#[test]
fn test_adv_match_size_exact_12() {
    assert_eq!(
        mem::size_of::<Match>(),
        12,
        "Match must be exactly 12 bytes"
    );
    assert_eq!(
        mem::align_of::<Match>(),
        4,
...
// 38. BlockMatcher trait generic implementation max block size
#[test]
...
// 39. BlockMatcher trait scan_block invocation
#[test]
...
// 40. BlockMatcher trait implementation check
#[test]
...
// 41. Error type completeness: InputTooLarge
#[test]
...
// 42. Error type completeness: MatchBufferOverflow
#[test]
...
// 43. Error type completeness: EmptyPatternSet
#[test]
...
// 44. Error type completeness: EmptyPattern
#[test]
...
// 45. Error type completeness: PatternCompilationFailed
#[test]
...
// 46. Error type completeness: Backend
#[test]
fn test_adv_error_backend() {
    let e = Error::Backend(Box::new(std::io::Error::new(
...
// 48. Zero-offset match contains self
#[test]
...
// 49. Zero-offset match overlaps logic
#[test]
...
// 51. Max-offset matches overlap
#[test]
fn test_adv_match_max_offsets_overlaps() {
    let m1 = Match::new(0, u32::MAX - 10, u32::MAX);
    let m2 = Match::new(0, u32::MAX - 20, u32::MAX - 5);
    assert!(m1.overlaps(&m2));
}

// 52. Overlapping match detection: exact match
#[test]
fn test_adv_match_overlap_exact() {
    let m1 = Match::new(0, 10, 20);
    let m2 = Match::new(0, 10, 20);
    assert!(m1.overlaps(&m2));
}

// 53. Overlapping match detection: one byte overlap
#[test]
fn test_adv_match_overlap_one_byte() {
    let m1 = Match::new(0, 10, 20);
    let m2 = Match::new(0, 19, 30);
    assert!(m1.overlaps(&m2));
    assert!(m2.overlaps(&m1));
}

// 54. Overlapping match detection: adjacent boundary (no overlap)
#[test]
fn test_adv_match_overlap_adjacent() {
    let m1 = Match::new(0, 10, 20);
    let m2 = Match::new(0, 20, 30);
    assert!(!m1.overlaps(&m2));
    assert!(!m2.overlaps(&m1));
}

// 55. Overlapping match detection: fully contained
#[test]
fn test_adv_match_overlap_contained() {
    let m1 = Match::new(0, 10, 50);
    let m2 = Match::new(0, 20, 30);
    assert!(m1.overlaps(&m2));
    assert!(m2.overlaps(&m1));
}

// 56. Overlapping match detection: max u32 bounds exact match
#[test]
fn test_adv_match_overlap_max_exact() {
    let m1 = Match::new(0, u32::MAX - 10, u32::MAX);
    let m2 = Match::new(0, u32::MAX - 10, u32::MAX);
    assert!(m1.overlaps(&m2));
}

// 57. Overlapping match detection: negative/inverted bounds (start > end)
#[test]
fn test_adv_match_overlap_inverted_bounds() {
    let m1 = Match::new(0, 50, 10);
    let m2 = Match::new(0, 0, 20);
    assert!(!m1.overlaps(&m2));
}

// 58. Merge overlapping: zero-offset matches behavior
#[test]
fn test_adv_merge_zero_offset_matches() {
    let mut s = MatchSet::new();
    s.insert(Match::new(0, 0, 0));
    s.insert(Match::new(0, 0, 0));
    s.merge_overlapping();
    assert_eq!(s.len(), 1);
}

// 59. Merge overlapping: chained adjacent matches
#[test]
fn test_adv_merge_chained_adjacent() {
    ...
    assert_eq!(
        s.len(),
        1,
        "Chained adjacent same-pattern matches should merge"
    );
    assert_eq!(s.as_slice()[0], Match::new(1, 0, 30));
}

// 60. GpuMatch layout exact fields verification
#[test]
fn test_adv_gpumatch_fields() {
    let g = GpuMatch::new(u32::MAX, 1, 2);
    assert_eq!(g.0[0], u32::MAX);
    assert_eq!(g.0[1], 1);
    assert_eq!(g.0[2], 2);
}

// 61. MatchOrd: sorting respects start then pattern_id
#[test]
fn test_adv_match_ord_rules() {
    let m1 = Match::new(10, 0, 10);
    let m2 = Match::new(5, 0, 10);
    assert!(m2 < m1);
}

// 62. MatchOrd: sorting respects end when start and pattern_id equal
#[test]
fn test_adv_match_ord_end_rules() {
    let m1 = Match::new(5, 0, 20);
    let m2 = Match::new(5, 0, 10);
    assert!(m2 < m1);
}

// 63. Error result type compatibility
#[test]
