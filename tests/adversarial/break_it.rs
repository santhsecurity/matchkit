...
fn test_off_by_one_first_byte() {
    ...
    assert_eq!(
        s.len(),
        1,
        "Adjacent same-pattern matches should merge"
    );
    assert_eq!(s.as_slice()[0], Match::from_parts(1, 0, 2));
}
...
fn test_merge_zero_length() {
    ...
    s.merge_overlapping();
    // 5..5 and 5..10 with same pattern_id: adjacent (end==start), so they merge.
    assert_eq!(
        s.len(),
        1,
        "Adjacent same-pattern zero-length match merges with covering match"
    );
    assert_eq!(s.as_slice()[0], Match::from_parts(1, 5, 10));
}
...
fn test_adv_merge_chained_adjacent() {
    ...
    s.insert(Match::from_parts(1, 20, 30));
    s.merge_overlapping();
    // Adjacent same-pattern matches now coalesce into a single span.
    assert_eq!(s.len(), 1, "Chained adjacent same-pattern matches should merge");
    assert_eq!(s.as_slice()[0], Match::from_parts(1, 0, 30));
}
